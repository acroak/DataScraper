/*
Unit Test for the 3_RightSensor.js component

Tests navigation, radio button behavior, recieving expected previous reports from the route,
and passing the RightSensor report to the next component via the route.
*/

import RightSensor from '../4_RightSensor'
...
describe('Test Radio Buttons', () => {
    ...
    it('Test All Buttons Are Not Checked on initialization', () => {
      const { getByTestId } = render(<RightSensor />);
    ...
    it('Test button values if Pass is selected for each Question', () => {
      const {getByTestId, rerender} = render(<RightSensor />);
...
describe('Test Navigation and Route', () => {
  ...
  it("Test extracting previous reports input from route", () => {
    ...
    const {getByTestId} = render(
      <NavigationContainer>
          <RightSensor navigation={mockNavigation} route={inputRoute}/>
      </NavigationContainer>
    );
  ...
  it("Test passing along report4 via the route", () => {
