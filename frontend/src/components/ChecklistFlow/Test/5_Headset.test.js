/*
Unit Test for the 5_Headset.js component

Tests navigation, radio button behavior, recieving expected previous reports from the route,
and passing the Headset report to the next component via the route.
*/

import Headset from '../5_Headset';
...
describe('Test Radio Buttons', () => {
    ...
    it('Test All Buttons Are Not Checked on initialization', () => {
      const { getByTestId } = render(<Headset />);
    ...
    it('Test button values if Pass is selected for each Question', () => {
      const {getByTestId, rerender} = render(<Headset />);
...
describe('Test Navigation and Route', () => {
  ...
  it("Test extracting previous reports input from route", () => {
    ...
    const {getByTestId} = render(
      <NavigationContainer>
          <Headset navigation={mockNavigation} route={inputRoute}/>
      </NavigationContainer>
    );
  ...
  it("Test passing along report5 via the route", () => {
