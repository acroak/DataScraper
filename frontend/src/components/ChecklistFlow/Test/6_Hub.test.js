/*
Unit Test for the 6_Hub.js component

Tests navigation, radio button behavior, recieving expected previous reports from the route,
and passing the Hub report to the next component via the route.
*/

import Hub from '../6_Hub'
...
describe('Test Radio Buttons', () => {
    ...
    it('Test button values if Pass is selected for each Question', () => {
      const {getByTestId, rerender} = render(<Hub />);
...
describe('Test Navigation and Route', () => {
  ...
  it("Test extracting previous reports input from route", () => {
    ...
    const {getByTestId} = render(
      <NavigationContainer>
          <Hub navigation={mockNavigation} route={inputRoute}/>
      </NavigationContainer>
    );
    ...
    expect(mockNavigation.navigate).toHaveBeenCalledWith(
      'Inspection Flow Complete', {
        report1: inputRoute.params.report1,
        report2: inputRoute.params.report2,
        report3: inputRoute.params.report3,
        report4: inputRoute.params.report4,
        report5: inputRoute.params.report5,
        report6: []
      }
    );
  });
  ...
  it("Test passing along report6 via the route", () => {
